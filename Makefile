...
container-crossbuild-binaries:
	...
	for ARCH in $(subst $(comma), ,$(CONTAINER_ARCHES)); do \
		docker cp $$(docker create --rm --platform $$(echo $$ARCH | cut -d'/' -f2) --name ece-tc $(CONTAINER_IMAGE_NAME):$(CONTAINER_IMAGE_TAG)):/bin/extended-ceph-exporter .output/ && \
			docker rm ece-tc; \
		mv .output/extended-ceph-exporter .output/extended-ceph-exporter-$$(echo $$ARCH | sed 's/\//_/g'); \
	done
